...
def _rust_walker(trie: TokenTrie, dfa: ScannerDFA, ignored: frozenset[int],
    ...
    # payload (GRID_PERF_KERNEL_LAZY); every other kernel entry gates on this
...
def walk(
    ...
    bound — they walk the Python specification below."""
    lazy = bool(getattr(dfa, "lazy", False))
    if _USE_RUST and len(priority) <= MAX_KERNEL_TERMINALS and (not lazy or _kernel_lazy_ok()):
        import numpy as np
