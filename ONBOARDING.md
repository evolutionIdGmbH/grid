...
| Serving: vLLM V1 backends + §6 contract (M6) | Mode 2 (logits processor, requires `async_scheduling=False`) and the **scheduler-side backend** (`grid/models/vllm_structured.py` + kernel v5 `fill_bits`) accepted on GPU (H100 + A10). Serving contract realized (`grid/serving/`): cold-only prefetch + E17 single-flight; G8 harness `bench/vllm_serving_bench.py` (warm-through cells, adversarial cold-miss, single-flight; unit-tested metrics); first-run +5151% batched-TPOT pathology diagnosed and fixed (LESSONS 6.5) | vllm 0.24, H100 declared runner, Qwen2.5-7B | `bench/RESULTS-serving.md` (kernel v6, H100 SXM5, 5-repeat, warm-through): G8 **5/7** — TPOT overhead **+1.02%** @batch 32 (+0.12% @1, +0.23% @8) **PASS** vs the <2% gate; TTFT cold 24.2 ms + warm 1.36 ms PASS; single-flight PASS; adversarial cold-miss pair, W10 H100 verdict (LESSONS 6.7/6.8): in artifact-free windows the full stack (defer + genN + rayon threads=8) PASSES both criteria — **−1.75% co-batched degradation, 23.9 ms max step** — on the lockstep leg that is biased AGAINST the design (conservative pass); defer attribution +8.2%/24.5 ms on vs +373%/65.7 ms off; fresh request TTFT ~145 ms at 1.00× warm effective TPOT. The v2 per-step record remains contaminated by a characterized EXOGENOUS vLLM-0.24 multiprocess artifact (once-per-leg 0.7–2 s frozen engine step; fires with zero grid work in all-warm baselines; invariant to every grid lever and to child/driver GC control; absent in-process) — upstream issue material. Admission warmup measured GIL-harmful and defaults OFF. Suite 238 green; T2 soundness fix (schema-scoped generic keys) retained |
