...
| Kernel v6 (session-in-kernel accept+fill) | `grid_core/src/lib.rs` sessions (own root→top chain, gen-tagged kidx cache, rollback deltas, ported maximal-munch lexer, memoized status/EOS derivation, (kidx,remainder)→handle bindings); serving gate: kernel present ∧ `audit is None` (audit/processor paths stay v5; `GRID_NO_V6=1` forces v5); differential suite `tests/models/test_v6_session.py` + 200-seed×3-grammar lockstep fuzz (zero divergence); also fixes the v5 post-COMPLETE resurrection (a non-eos token after eos revived a terminated request under spec decode) | local M-series, Qwen 151k trie | warm serving step 7.39 → **1.33 µs**/request (accept 6.16 → 0.56, fill 1.22 → 0.77); ≪ the 6 µs G8 per-request budget; H100 rerun pending |
...
| Serving: vLLM V1 backends + §6 contract (M6) | Mode 2 (logits processor, requires `async_scheduling=False`) and the **scheduler-side backend** (`grid/models/vllm_structured.py` + kernel v5 `fill_bits`) accepted on GPU (H100 + A10). Serving contract realized (`grid/serving/`): cold-only prefetch + E17 single-flight; G8 harness `bench/vllm_serving_bench.py` (warm-through cells, adversarial cold-miss, single-flight; unit-tested metrics); first-run +5151% batched-TPOT pathology diagnosed and fixed (LESSONS 6.5) | vllm 0.24, H100 declared runner, Qwen2.5-7B | `bench/RESULTS-serving.md` (5-repeat, warm-through): G8 **5/7** — TTFT cold 44.5 ms + warm 2.50 ms PASS, single-flight PASS; TPOT @32 **+10.7%** vs <2% gate (per-request Python orchestration ~30 µs/req/step vs the xgrammar floor +1.07%, `bench/vllm_xgr_floor.py` — kernel v6 session-in-kernel accept+fill is the path); adversarial max step 278 ms vs 30 ms budget — T2 tier landed but covers only revisited configs (giant keys include the literal remainder); residual = genuinely cold 100-300 ms literal-interior walks, and vLLM 0.24 has no RUNNING-request defer hook (kernel walk-cost attack, upstream defer hook, or criterion re-scope) |
