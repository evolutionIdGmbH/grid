...
| `GRID_LALR_BUDGET` | 32000000 | LALR construction item budget (P5; state cap derived at `//8` = 4M): both constructions count items materialized at state creation and crossing either cap raises the declared, deterministic `LALRBudgetExceeded` instead of hanging — the 2 residual LALR-family 120s compile caps (helm-testsuite, o27148) declare in ~29s/~42s at identical counts every run; calibrated full-corpus (largest completing build o21112 = 20.09M items; zero fires elsewhere); `=0` disables both caps (audit/oracle escape hatch), `50000000` recovers o27148's real conflict report (~132s) |
...
| `GRID_PERF_STORE_TRIE` | on (under the store master) | S3 sub-flag: TokenTrie namespace keyed by (tokenizer fingerprint, `GRID_PERF_SLICER` variant — S2 bakes TrieSlices into the payload at build time, so the flag states never share an entry); `=0` restores the per-process trie build |
